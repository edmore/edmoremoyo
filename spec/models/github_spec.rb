...
describe Github do
  ...
  describe "#list_commits" do
    ...
    it "should display a message when something goes wrong" do
      ...
      project.stub(:open_api).with("#{Github.api_url}/commits/list/#{@project.user}/#{@project.repo}/#{@project.branch}").and_return("Oops!!, some http issues, sorry.")
  ...
  describe "#commits_on_file" do
    it "should return the commits for a particular file" do
      file_name = "test_file"
      @project.stub(:open_api).with("#{Github.api_url}/commits/list/#{@project.user}/#{@project.repo}/#{@project.branch}/#{file_name}").and_return("commits_on_file")
      @project.commits_on_file(file_name).should == "commits_on_file"
    end

    it "should display a message when something goes wrong" do
      file_name = "test_file"
      @project.stub(:open_api).with("#{Github.api_url}/commits/list/#{@project.user}/#{@project.repo}/#{@project.branch}/#{file_name}").and_return("Something went wrong")
  ...
  describe "#commit" do
    it "should return a particular commit" do
      id = "abcxyz"
      @project.stub(:open_api).with("#{Github.api_url}/commits/show/#{@project.user}/#{@project.repo}/#{id}").and_return("commit")
      @project.commit(id).should == "commit"
    end

    it "should display a message when something goes wrong" do
      id = "abcxyz"
      @project.stub(:open_api).with("#{Github.api_url}/commits/show/#{@project.user}/#{@project.repo}/#{id}").and_return("Something went very wrong")
